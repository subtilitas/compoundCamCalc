...
- With rigid cords the first stop is the wall (computed, slice 10a). The
  ...
  about 1 µm past x₁.
- With cord stretch (slice 11a) the draw continues past x₁ to the second
  stop x₂. The force rise between x₁ and x₂ is the two-stage wall that
  archers report: on the default design with EA = 3e5 N and the top cable
  1 mm longer, 68.4 N at x₁ rising to 103.1 N at x₂, 5.21 mm later
  (computed, [model.md](model.md#elastic-cords)).
...
- **Wall stiffness with the cam on its stop.** The half string acts in
  ...
  The analysis of slice 11a gives 480.85 N/mm at the stop of the default
  design, 0.10 % above the formula evaluated there.
...
| 10b | Timing user interface. Settings group "Timing (analysis only)": top cable length change, bottom cable length change, string length change, nocking point above centre (mm). Results block: Δθ at the end of the draw (the first stop with rigid cords), first stop and gap at the other, nock travel, Δ brace height, Δ draw length, Δ peak, Δ let-off, dΔθ/dL. Timing chart: stacked nock height and Δθ over draw length. Glossary: timing, nock travel, draw stop. Results footnote, report note, user guide and Limitations updated. | Timing edits never change the cam, status or exports; one undo step per edit; share link keeps the offsets | Unit: glossary and guide text, defaults, ranges; Playwright: +1 mm top cable shows Δθ and the stop order, undo, share link, 320 px, axe | 0.4 |
| 10c | String plan with both halves from the analysis pose (brace and x₁); separate timing files (decision of the user): a timing string plan (DXF) and a timing table (CSV) with θ_t, θ_b, y and four tensions, design files unchanged; print report timing section | Drawings match the analysis poses to 1e-9 m | Layout pose tests, DXF round trip, report | 0.25 |
| 11 | Cord stiffness, in two parts: 11a the analysis core, 11b the settings (material and strand count per cord, rigid by default; decisions of the user), the Timing panel rows and the build lengths. EA per cord (string, top cable, bottom cable) with a rigid option (`cordModel: rigid | elastic`, no Infinity in JSON). Compliant closures: quasi-Newton before the stops, full Newton with ∂T/∂q at the wall. Free lengths L0 = L − C·T at brace. The draw continues past the first stop to the second stop x₂. Wall stiffness; build lengths free and at 445 N (100 lbf). Material presets from measured data only (452X at 12.36 kN per strand). | EA → ∞ reproduces 10a; equal EA keeps y = 0; differential EA gives Δθ; wall stiffness within 1 % of the closed form. Released as 0.3.0 | Rigid limit, energy balance with ½·C·T², wall at the stop, convergence at the wall, median below 90 ms for the analysis with its reference, the exact elastic Jacobian per sample included | 0.4 |

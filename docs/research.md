...
- With a free nock, the nock then slides vertically until the second stop
  at x₂.
- The force rise between x₁ and x₂ is the two-stage wall that archers
  report.

The analysis grid ends at x₂, which can lie beyond x_f.
...
|---|---|---|---|---|
| 9 | Forward compatibility. `migrate` reports dropped unknown keys ("Settings this version does not know were left out: …"). The newer-version messages point to the Version select. The design id of export file names ignores analysis-only inputs. The `solve` option `analysis` is added, off by default. | Released as a tag before slice 10b | Unit: dropped-key notice for files and links, messages, design id stable under an added analysis section; Playwright: open a file with an unknown key | 0.1 |
| 10a | `src/core/analysis.js`, rigid and asymmetric, no user interface. Poses per half in the mirror frame. 4×4 Newton on the closures at fixed (x, y); secant on y for F_y = 0. Tensions from Jᵀ T = −∇E. Brace re-solved with x, y, θ and α free. Stop gaps with event bisection; grid to the second stop, 300 samples. Outputs y, θ_t, θ_b, Δθ, F, F_y, four tensions, gaps, k_y, dΔθ/dL. `analysis-*` diagnostics: slack cord, fold, wrap at brace, k_y ≤ 0. Section in model.md. | Zero offsets reproduce `solveForward` to 1e-9 relative on the default and the nine samples; median below 25 ms | Regression; equal offsets keep y = 0 to 1e-12; swapped offsets mirror y and flip Δθ; free-body balance per half; ∫F dx = ΔE; dΔθ/dL against the closed form; stop events; one test per diagnostic; performance in a worker | 0.45 |
...
| 13 | Open spiral tracks. A track may end at a post with a step in the outline instead of the closing blend: the working arc stays convex, the outline is closed by a radial step and the post. Check that no free cord span touches any part of the outline it does not wrap, on either track and the step included, at every draw position, lead-in and residual wrap included: with an open track, a convex working arc no longer guarantees this. Minimum wall at the step corner. Middle plate and cam overlap without the convexity assumption (distance between outlines). DXF and STEP outlines with the step. The cable track may end at a post whose cord line passes closer to the axle than bore radius + wall + d/2 at full draw, but every free cord span keeps its centreline at least bore radius + d/2 plus a set clearance from the axle centre over the whole draw, so no cord touches the axle. A termination post near the axle keeps at least the minimum wall to the axle bore and to every other hole; a post that cannot is refused with a diagnostic instead of being moved. | A spiral track with a step builds with zero diagnostics; convex tracks export unchanged | Clearance of every free span to the whole outline (step and unwrapped arcs), free-span clearance to the axle and post-to-bore wall, each with a boundary case on each side of the limit, overlap on non-convex outlines, export round trips, cable lever arm below the bore-and-wall limit | 0.45 |
